...
protected:
	...
	int MaxTrials; //максимальное число испытаний
...
class TPiyavskogoMethod : public IMethod
{
   protected:
	...
	\param[in] p указатель на интервал, характеристику которого надо вычислить, M константа липшица.
	\return Характеристика интервала
	*/
	   double CalculateR(int p);//, double M
	   /** Вычисление оценки константы Липшица
	   ...
	   */
	   ...
	   bool UpdateOptimumEstimation(const TTrial& trial) = 0;



	   /// Проверяет попала ли точка в окрестность глобального манимума , что за окрестность?
	   bool CheckOptimumPoint(const TTrial& trial) = 0;

	   /** Вычисления точки очередной итерации

	Результат записывается в CurTrial-------------------------------ее создать самому?
	*/
	   virtual void CalculateIterationPoint() = 0;
	   /** Проверка выполнения критерия остановки метода
